...
def train_kd_one_epoch(model, t_model, data, epoch, loss, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            ...
            quant_loss = model.module.get_quant_loss(images)
            ...
            total_loss = task_loss + ckd_loss + icl_loss + cross_kd_loss + fd_loss + gd_loss + afd_loss + quant_loss
        ...
        if is_master(args) and (i % 100 == 0 or batch_count == num_batches_per_epoch):
            ...
            loss_quant.update(quant_loss.item(), batch_size)
            logit_scale_scalar = logit_scale.item()
            logging.info(
                f"Train Epoch: {epoch} [{num_samples:>{sample_digits}}/{samples_per_epoch} ({percent_complete:.0f}%)] "
                ...
                f"Quant Loss: {loss_quant.val:#.5g} ({loss_quant.avg:#.4g}) "
                f"Data (t): {data_time_m.avg:.3f} "
                f"Batch (t): {batch_time_m.avg:.3f}, {args.batch_size*args.world_size / batch_time_m.val:#g}/s "
                f"LR: {optimizer.param_groups[0]['lr']:5f} "
                f"Logit Scale: {logit_scale_scalar:.3f}"
